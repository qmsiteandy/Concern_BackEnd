...
classroomRouter.post(
  ...
  expressAsyncHandler(async (req, res, next) => {
    ...
    if (classroom) {
      ...
      //計算缺席人數
      result.absenceCount =
        result.shouldAttendCount -
        result.attentCount -
        result.personalLeaveCount;
    ...
    if (classroom) {
      if (classroom.classmates.length > 0) {
        ...
        classroom.classmates.forEach((classmate) => {
          ...
          let concernPercentageString =
            Math.floor(concernPercentage * 100) + "%";
        ...
        for (let rank = 0; rank < newRankCount; rank++) {
          ...
          for (let i = rank + 1; i < classmateData.length; i++) {
            if (
              classmateData[bestIndex].aveConcern < classmateData[i].aveConcern
            )
              bestIndex = i;
          }
        ...
        for (let rank = 0; rank < newRankCount; rank++) {
          ...
          for (let i = rank + 1; i < classmateData.length; i++) {
            if (
              classmateData[bestIndex].bestLasted < classmateData[i].bestLasted
            )
              bestIndex = i;
          }
          let storage = classmateData[bestIndex];
